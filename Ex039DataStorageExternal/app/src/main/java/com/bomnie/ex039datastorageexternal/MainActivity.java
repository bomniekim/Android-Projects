package com.bomnie.ex039datastorageexternal;
...
public class MainActivity extends AppCompatActivity {
    ...
    public void clickSave(View view) {
        ...
        //여기까지 커서가 오면 외장메모리가 연결되어있는 것
        // 외부메모리에 데이터 저장 작업 시작

        String data= et.getText().toString();
        et.setText("");

        File path; // 파일에 저장될 경로정보를 가진 객체 [Data.txt 파일이 저장될 경로]

        //Marshmallow(api 23버전) 이상에서는
        // SD 카드의 아무 위치에 직접 저장하는 것이
        //불가능하도록 보안을 강화함 (동적퍼미션을 받아야 가능함)
        //오로지 각 앱에게 할당된 고유한 영역에다만 저장이 가능
        // 그 고유한 영역의 경로를 얻어오기!
        File[] dirs= getExternalFilesDirs("MyDir");
        path= dirs[0];
        tv.setText( path.getPath() );

        //위에서 만들어진 경로에 Data.txt 문서를
        //만들고 데이터를 저장
    ...
    //requestPermission()을 실행해서 보여진 permission 요청 dialog 의
    // allow 또는 deny 중 하나를 선택하면 자동으로 실행되는 콜백메소드
    @Override
        ...
        switch (requestCode) {
            case 100:
                ...
                if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    Toast.makeText(this, "외부 저장소 사용가능", Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(this, "거부 시 외부 저장소 사용 불가", Toast.LENGTH_SHORT).show();
                }
    ...
    public void clickBtn (View view) {
        //외부메모리의 특정 위치에 저장하려면 동적 퍼미션이 필요 (api23버전 이상부터)
        ...
        //퍼미션이 허가되었다면 외부메모리에 저장하기

            //SD 카드의 특정 위치에 aaa.txt  문서 저장
